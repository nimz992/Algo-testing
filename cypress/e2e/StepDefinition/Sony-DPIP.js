...
if (
  Cypress.spec.name === "List Ask.feature" ||
  ...
  Then("User change week 1 ship override", () => {
    cy.get(":nth-child(5) > .form-group > .ng-untouched")
      .clear()
      .type("0.0336");
  });
  ...
  Then(
    "User waits for 'list raw material subclass lead times' Response",
    () => {
      // Checks app-dynamic-tabs for response.
      cy.get(`[id="Panel - 01"] app-dynamic-tabs`, {
        timeout: 500000,
  ...
  When(
    "User ask 'strategic forecast planning calendar fiscal year 2030'",
    () => {
      cy.get("#mat-input-0", {
        timeout: 10000,
